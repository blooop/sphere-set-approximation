...
{
	auto t = overlap_ratio(sphere, 1000000);
...
{
	auto t = unique_volume(sphere, 1000000);
...
std::vector<Sphere> sphere_set_approximate(const std::string &pt_output_file, const RTcore::Mesh& originalmesh, std::string visualizer_mesh_filename, const RTcore::Mesh& manifold, int ns, int ninner, int nsurface, int n_finalsample, int n_mutate)
{
	...
	visualize_with_mesh(surfacepoints, visualizer_mesh_filename, 0.02);
	...
	auto checkresult = [&](const std::vector<Sphere>& sphere){
		...
		if (sumloss < bestsumloss) {
			visualize(sphere, visualizer_mesh_filename, pt_output_file);
			bestsumloss = sumloss;
			bestresult = sphere;
		}
		return sumloss;
	...
	// final expanding
	visualize(bestresult, visualizer_mesh_filename, pt_output_file);
